...
function get_security_questions_list()
{
    return [
        1 => '좋아하는 동물은 무었입니까?',
        2 => '보고있으면 좋아지는 색깔은 무었입니까?',
        3 => '서울시와 세종시 중에 어느 곳에서 살고 싶습니까?',
        4 => '최근에 읽은 책 제목은 무엇입니까?',
        5 => '기호식품중 추천하고 싶은 것은 무엇입니까?'
    ];
}

function error_msg($input = false){    global $error_error;    if (!empty($input)) {        $_SESSION['error_msg'] = $input;    } elseif (!empty($_SESSION['error_msg'])) {        echo "<p id=\"error-message\" class=\"alert alert-danger\" style=\"position: fixed; top: 0; left: 0; width: 100%; z-index: 10000; text-align: center; box-sizing: border-box;\">" . $_SESSION['error_msg'] . "</p>";        echo "<script>setTimeout(function() { var elem = document.getElementById('error-message'); if (elem) elem.style.display = 'none'; }, 3000);</script>";        unset($_SESSION['error_msg']);    }}function success_msg($input = false){    global $success_msg;    if (!empty($input)) {        $_SESSION['success_msg'] = $input;    } elseif (!empty($_SESSION['success_msg'])) {        echo "<p id=\"success-message\" class=\"alert alert-success\" style=\"position: fixed; top: 0; left: 0; width: 100%; z-index: 10000; text-align: center; box-sizing: border-box;\">" . $_SESSION['success_msg'] . "</p>";        echo "<script>setTimeout(function() { var elem = document.getElementById('success-message'); if (elem) elem.style.display = 'none'; }, 3000);</script>";        unset($_SESSION['success_msg']);    }}
...
{
    // Temporarily bypass CAPTCHA validation for debugging.
...
{
    // Log the generated captcha phrase for server-side debugging
    error_log("Generated Captcha Phrase (DEBUG): " . user::$captcha->getPhrase());
    ...
    if (!empty(get_config('captcha_type'))) {
        if (get_config('captcha_type') == 1) {
            ...
            // This branch is for captcha_type > 2 or other cases, which should return empty string for no captcha
            return '';
        }
    }

    // This block is for captcha_type == 0 (Image Captcha)
    if(empty($bootstrap))
    {
        return '<div class="input-group"><input type="text" placeholder="' . lang('captcha') . '" name="captcha"></div><p class="captcha-image-container" style="text-align: center;margin-top: 10px;"><img src="' . user::$captcha->inline() . '" style="border - radius: 5px;"/></p>';
    }

    return '<div class="input-group"><span class="input-group">' . lang('captcha') . '</span><input type="text" class="form-control" placeholder="' . lang('captcha') . '" name="captcha"></div><p class="captcha-image-container" style="text-align: center;margin-top: 10px;"><img src="' . user::$captcha->inline() . '" style="border - radius: 5px;"/></p>';
}
...
{
    // calculate verifier using this salt
    $calculated_verifier = calculateSRP6VerifierBnetV2($email, $pass, $salt);
...
function get_template_image_url($image_path)
{
    global $antiXss;
    return $antiXss->xss_clean(get_config("baseurl")) . '/template/' . $antiXss->xss_clean(get_config("template")) . '/' . $image_path . '?v=' . time();
}
